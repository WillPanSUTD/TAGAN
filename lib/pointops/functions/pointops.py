...
# 修改
# 这2行代码不是保存模型的代码，而是创建日志和模型保存目录的代码。
# 简单来说，它们的作用是“建房子”，而不是“把东西搬进去”。
sys.path.append('/userData/gpulij/sewer3d/lib/pointops/functions')
sys.path.append('/userData/gpulij/sewer3d')
...
# 报错原因很简单：pointops_cuda 是一个需要编译的 C++/CUDA 扩展库。
# 如果你没有运行 python setup.py install 进行编译
# 或者编译过程因环境问题（CUDA 版本不匹配、缺少编译器等）失败了，Python 就找不到这个模块。
try:
    import pointops_cuda
except ImportError:
    import pointops.pointops_cuda as pointops_cuda
...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        idx = torch.zeros((new_offset[b - 1].item()), dtype=torch.int, device='cuda')
        tmp = torch.ones(n, dtype=torch.float, device='cuda') * 1e10
...
class Grouping(Function):
    ...
    def forward(ctx, input, idx):
        ...
        output = torch.zeros((m, nsample, c), dtype=torch.float, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((n, c), dtype=torch.float, device='cuda')
...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True, edge=False):
    """
    ...
    if new_xyz is None:
        new_xyz = xyz
    if idx is None:
        idx, distance = knnquery(nsample, xyz, new_xyz, offset, new_offset)  # (m, nsample)
    ...
    if edge:
        grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c) - feat.unsqueeze(1)
        # grouped_feat[:, 1:, :] = grouped_feat[:, 1:, :] - feat.unsqueeze(1)
    else:
        grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c)  # (m, nsample, c)
    # grouped_feat = grouping(feat, idx)  # (m, nsample, c)

    if use_xyz:
        return torch.cat((grouped_xyz, grouped_feat), -1), distance  # (m, nsample, 3+c)
    else:
        return grouped_feat, distance


def queryandgroup_nor(nsample, xyz, new_xyz, normal, feat, idx, offset, new_offset, use_xyz=False, use_normal=True, edge=False):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b)
    output: new_feat: (m, c+3, nsample), grouped_idx: (m, nsample)
    """
    ...
    n, m, c = xyz.shape[0], new_xyz.shape[0], feat.shape[1]
    grouped_xyz = xyz[idx.view(-1).long(), :].view(m, nsample, 3)  # (m, nsample, 3)
    grouped_normal = normal[idx.view(-1).long(), :].view(m, nsample, 3)  # (m, nsample, 3)
    grouped_xyz -= new_xyz.unsqueeze(1)  # (m, nsample, 3)
    grouped_normal -= normal.unsqueeze(1)  # (m, nsample, 3)
    if edge:
        grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c) - feat.unsqueeze(1)
    else:
        grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c)  # (m, nsample, c)
    ...
    if use_xyz:
        ...
        return torch.cat((grouped_normal, grouped_feat), -1), distance  # (m, nsample, 3+c)
    else:
        return grouped_feat, distance
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        n, c = input1.shape;
        nsample = idx.shape[-1]
        output = torch.zeros((n, nsample, c), dtype=torch.float, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros((n, c), dtype=torch.float, device='cuda')
        grad_input2 = torch.zeros((n, c), dtype=torch.float, device='cuda')
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        w_c = weight.shape[-1]
        output = torch.zeros((n, c), dtype=torch.float, device='cuda')
    ...
    def backward(ctx, grad_output):
        ...
        w_c = weight.shape[-1]
        grad_input = torch.zeros((n, c), dtype=torch.float, device='cuda')
        grad_position = torch.zeros((n, nsample, c), dtype=torch.float, device='cuda')
        grad_weight = torch.zeros((n, nsample, c), dtype=torch.float, device='cuda')
        # grad_input = torch.cuda.FloatTensor(n, c).zero_()
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    new_feat = torch.zeros((new_xyz.shape[0], feat.shape[1]), dtype=torch.float, device='cuda')
...
class Interpolation(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros((m, c), dtype=torch.float, device='cuda')
